		...
		var area16 = ["서귀포시","제주시"];
		 ...
		 $("#btnApplication").click(function(){
				...
				if($('input[name=child]:checked').length == 0){
					alert("수혜자 아동을 선택해주세요.");
				} else {
					var tr = $('input[name=child]:checked').parent().parent();
					var td = tr.children();
					var aperson = td.eq(1).text()+'/'+td.eq(2).text()+'/'+td.eq(3).text();
					
					$('input#aperson').val(aperson);
					
					facilityApplyForm.submit();
				}
			});
